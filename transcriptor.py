...
valid_audio_extensions = ['.mp3', '.m4a', '.wav', '.flac', '.ogg', '.aac']

# Functions


def check_audio_file(file_path):
    """Check that file is a recognised audio file type"""
    ...
    if not os.path.isfile(file_path):
        print(f"""Error: {
              file_path} is not a file. / Erreur : {file_path} n'est pas un fichier.""")
        sys.exit(1)

    if not file_path.endswith(tuple(valid_audio_extensions)):
        print(textwrap.fill(
...
def replace_extension(file_name):
    ...
    for extension in valid_audio_extensions:
        new_file_name = file_name.replace(extension, '.txt')
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print("Usage: python transcriptor.py <audio_file.m4a> [--dry-run]")
        sys.exit(1)
    ...
    if not dry_run:
        ...
        print(center_text(
            "Dry run mode: Skipping transcription and file writing. / Mode test : Transcription et écriture de fichier sautées."))
